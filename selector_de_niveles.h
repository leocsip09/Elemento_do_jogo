...
private:
    ...
    sf::Text jugador1, jugador2, advertencia;
    std::string seleccionJ1, seleccionJ2, mensaje;
...
public:
        ...
        while (SeleNiv->pollEvent(event)) {
            ...
            if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
                ...
                if (Alfalfa1->getGlobalBounds().contains(mouse_coord)) {
                    seleccionJ1 = "Alfalfa";
                    std::cout << "Jugador 1 Selecciona a " << seleccionJ1 << '\n';
                    ...
                    seleccionJ2 = "Alfalfa";
                    std::cout << "Jugador 2 Selecciona a " << seleccionJ2 << '\n';
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Enter) && !laseleccion) {
                if (seleccionJ1 != "" && seleccionJ2 != "") {
                    ...
                    mensaje = "Faltan escoger personajes";
    ...
    void run_Selector() {
        sf::Clock clock;  // Declare this clock outside the loop
        while (SeleNiv->isOpen()) {
            eventos_bucle();
            float deltaTime = clock.restart().asSeconds();  // Calculate deltaTime
            if (laseleccion && pos >= 1) {
                Nivel* nivelSeleccionado = MultitonNivel::getNivel(pos, seleccionJ1, seleccionJ2);

                // Call the actualizar functions of characters here
                //personajeJugador1->actualizar(deltaTime);
                //personajeJugador2->actualizar(deltaTime);
    ...
    void estab_val_Sel() {
        ...
        advertencia.setFont(*fuente2);
        advertencia.setString(mensaje);
        advertencia.setCharacterSize(25);
        advertencia.setOutlineColor(sf::Color::Black);
        advertencia.setOutlineThickness(4);
        advertencia.setPosition({ 600, 300 });
